...
def stripe_noise_filter(data: xr.DataArray,cutoff, notch_width,radius,horisontal_filter=False ) -> xr.DataArray:
    ...
    """
    Apply a 2D Fourier filter to remove stripe noise from sonar data.  
    The filter is applied in the frequency domain, and the filtered data is returned.
    Parameters:
        data (xr.DataArray): The input sonar data.
        cutoff (int): The cutoff index for the range dimension.
        notch_width (int): The width of the notch filter.
        radius (int): The radius of the circular mask in the frequency domain.
        horisontal_filter (bool): If True, apply a horizontal filter as well.
    Returns:
        xr.DataArray: The filtered sonar data.
    """
